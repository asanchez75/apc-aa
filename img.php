...
ob_start();
if (!include_once(AA_PHPTHUMB_PATH.'phpthumb.class.php')) {  // Honza AA Change
    ob_end_flush();
...
require_once AA_PHPTHUMB_PATH.'phpthumb.config.php';  // Honza AA Change
...
if (!@$PHPTHUMB_CONFIG['disable_pathinfo_parsing'] && (empty($_GET) || isset($_GET['phpThumbDebug'])) && !empty($_SERVER['PATH_INFO'])) {
    ...
    for ( $i=0, $ino=count($args) - 2; $i<$ino; ++$i) {
