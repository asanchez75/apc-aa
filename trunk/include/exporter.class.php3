...
class AA_Exportsetings extends AA_Object {
    ...
    function export($restict_zids=null) {
        ...
        $set = new AA_Set($slice_id, AA_Stringexpand::unalias($this->conds), AA_Stringexpand::unalias($this->sort), $this->bins);
        $fs  = new AA_Fieldset;
        ...
        $exporter = AA_Object::factory($this->format, array('field_set'=>$fs, 'grabber'=>$grabber));
        if (is_null($exporter)) {
