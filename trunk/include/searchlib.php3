...
function SearchWhere ($search, $srchflds) {
   ...
   if( ($foo=date2datetime($search[from])) != "" )
     $where .= " AND publish_date >= '$foo'";
   if( ($foo=date2datetime($search[to])) != "" )
     $where .= " AND publish_date <= '$foo 23:59:59'";
   return $where;
}
...
function MakeWhere($p_slice_id, $cat, $high) {
  $where = "(slice_id='$p_slice_id' AND ".  
           "publish_date <= '". date("Y-m-d"). " 23:59:59' AND ".  //if you change bounds, change it in table.php3 too
           "expiry_date > '". date("Y-m-d"). "' AND ".             //if you change bounds, change it in table.php3 too
