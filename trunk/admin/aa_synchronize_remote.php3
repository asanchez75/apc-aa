...
if (!IsSuperadmin()) {
    $response = new AA_RemoteResponse(_m("You don't have permissions to synchronize slices."), 101);  // error code > 0
    $response->respond();
    exit;
}

$class2run = 'AA_Remote_'. $_REQUEST['cmd'];
if ( class_exists($class2run) ) {
    $response = call_user_func(array($class2run, 'run'));
} else {
    $response = new AA_RemoteResponse(_m('No such command'), 100);  // error code > 0
}
$response->respond();
exit;
