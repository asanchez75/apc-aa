...
if( $view_templates ) {
    ...
    if ($view_templates_rewrite_existing) {
        ...
        foreach($select as $key => $val) {
            $SQL = "UPDATE ".$SQL_view_templates[$key]." WHERE id='".$val."'";
            safe_echo( $SQL );
            myquery($db, $SQL );
        }
    }
    ...
    reset($select);
    foreach ($select as $key=>$val) {
        if ($SQL_view_templates[$select[$key]] == "") {
