...
class AA_Optimize_Add_Mandatory_Status_Code extends AA_Optimize {
    ...
    function test() {
        ...
        $row_count   = GetTable2Array("SELECT count(*) as count FROM slice LEFT JOIN field ON (slice.id=field.slice_id AND field.id='".$this->_field()."') WHERE field.id IS NULL", "aa_first", 'count');
        if ($row_count > 0) {
